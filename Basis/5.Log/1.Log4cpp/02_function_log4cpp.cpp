...
int main(int argc, char** argv) {
    ...
    /* 则 sub1 和 sub10 引用同一个记录器 sub1 */

    /* 创建记录器 如果是 root 的子记录器 则 getInstance() 参数写子记录器名称即可
     * 如果想创建子记录器的子记录器 即 sub1 的 子记录器 需要 getInstance("sub1.sub11")
     */
     ...
     * */
    /* sub1.setAdditivity(false); /1* 只在当前日志记录器中处理, 不会向上传递给夫日志记录器 *1/ */
